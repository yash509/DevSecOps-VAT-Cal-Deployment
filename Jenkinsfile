pipeline {
    ...
    stages {
        ...
        stage('Checkout from Git') {                        
            steps {                                       
                git branch: 'main', url: 'https://github.com/yash509/DevSecOps-VAT-Cal-Deployment.git'
            }
        }
        ...
        stage("Sonarqube Analysis ") {                         
            steps {
                //dir('Band Website') {
                    ...
                    sh '''$SCANNER_HOME/bin/sonar-scanner -Dsonar.projectName=vat-application \
                    -Dsonar.projectKey=vat-application'''
                    //}
                }
            }
        }
        ...
        stage("Docker Image Tagging"){
            steps{
                script{
                    //dir('Band Website') {
                        withDockerRegistry(credentialsId: 'docker', toolName: 'docker'){
                            sh "docker tag vat-calc yash5090/vat-calc:latest " 
                        //}
                    }
                }
            }
        ...
        stage('Docker Image Scanning') { 
            steps { 
                sh "trivy image --format table -o trivy-image-report.html yash5090/vat-calc:latest" 
            } 
        } 
        ...
        stage("Image Push to DockerHub") {
            steps{
                script{
                    //dir('Band Website') {
                        withDockerRegistry(credentialsId: 'docker', toolName: 'docker') {
                            sh "docker push yash5090/vat-calc:latest "
                        //}
                    }
                }
            }
        ...
        stage('Docker Scout Image Scanning') {
            steps {
                script{
                   withDockerRegistry(credentialsId: 'docker', toolName: 'docker'){
                       sh 'docker-scout quickview yash5090/vat-calc:latest'
                       sh 'docker-scout cves yash5090/vat-calc:latest'
                       sh 'docker-scout recommendations yash5090/vat-calc:latest'
                       sh 'docker-scout attestation yash5090/vat-calc:latest'
                   }
                }   
            }
        }
        ...
        stage("TRIVY"){
            steps{
                //dir('Band Website') {
                    sh "trivy image yash5090/vat-calc:latest > trivyimage.txt"   
                //}
            }
        }
        ...
        stage('Deploy to Docker Container'){
            steps{
                //dir('BMI Calculator (JS)') {
                    sh 'docker run -d --name vat-calc -p 5000:80 yash5090/vat-calc:latest' 
                //}
            }
        }
